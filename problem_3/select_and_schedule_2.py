...
def get_nearest_site(fc, p, sites):
    min_p = ""
    min_cost = 1000000
    name_index, index_name = get_name_index() 
    for i in range(0, len(fc[0])):
        if index_name[i] not in sites:
            continue
        if fc[name_index[p]][i] < min_cost:
            min_cost = fc[name_index[p]][i] 
            min_p = index_name[i]

    return min_p, min_cost

def deal_added_cars(sites):
    fci = "./combinations_3.dat"
    fcc = "./combinations_c.dat"
    ...
    while line:
        ...
        tmp_sites = []
        for elem in sites:
            tmp_sites.append(elem)
        ps = line.split(' ')
        for p in ps:
            dst, cost = get_nearest_site(fc, p, tmp_sites)
            tmp_sites.remove(dst)
            s += " " + dst + "," + str(cost)
            total_cost += cost
        if len(ps) == 2:
            dst, cost = get_nearest_site(fc, p, tmp_sites)
            ...
            total_cost += cost
        line = fpi.readline()
        fpc.write(s + " " + str(total_cost) + "\n")

    fpi.close()
    fpc.close()
 
def remove_sites(select_file, removed_cars, sites):
    fp = open(select_file)
    line = fp.readline()
    cars_sites = {}
    while line:
        line = line.strip()
        tmp = line.split(" ")
        cars_sites[tmp[0]] = tmp[2]
        line = fp.readline()
    fp.close()
    for car in removed_cars:
        sites.remove(cars_sites[car])
    
    return sites
        
def schedule(removed_cars, deal_addition, num):
    ...
    ##########################################################
    goal = {}
    goal_count = 24 - len(removed_cars)
    sites = get_available(zqueue)
    if len(removed_cars) != 0:
        sites = remove_sites(select_file, removed_cars, sites)
    ...
    for z in zarray:
        for path in zqueue[z]:
            ...
            fp_sche.write(s + "\n") 
            if path[0][0] == 'C':
                sites_cost.append([path[0], leave])
    ...
    sites_cost.sort(key=lambda elem: elem[1], reverse=True)
    #print sites_cost

    return [sites_cost[i][0] for i in range(0, num)]
    
def get_slowest_cars(count):
    return schedule([], 0, count)

def main():
    del_count = 3
    slowest_cars = get_slowest_cars(del_count)
    print "\nShould be replaced cars:", slowest_cars, "\n"
    schedule(slowest_cars, 1, 0)
